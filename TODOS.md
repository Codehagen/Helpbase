...
_All P1 items for the 2026-04-15 knowledge-layer plan shipped._
...
### TODO-018: `helpbase context` — direct `@ai-sdk/anthropic` / `@ai-sdk/openai` BYOK
**Completed:** 2026-04-17 — `packages/shared/src/llm.ts` now exports `resolveByokModel(modelString)` which routes by env-var precedence: `AI_GATEWAY_API_KEY` > `ANTHROPIC_API_KEY` > `OPENAI_API_KEY` > hosted. When a provider-specific key is set, the model string must match the provider prefix (e.g. `anthropic/claude-3-5-sonnet-latest` with `ANTHROPIC_API_KEY`); mismatches throw a clear `GatewayError` with the exact command to try instead. `isByokMode()` now returns true for any of the three keys. `byokGenerateObject` + `byokGenerateText` pass the resolved model factory to the Vercel AI SDK. Error catalog (`context-errors.ts::E_CONTEXT_MISSING_KEY`), `helpbase whoami`, and `helpbase doctor` all document the three options. 141/141 shared tests green (9 new routing tests covering every precedence path + error case). Pinned `@ai-sdk/anthropic@^2.0.77` + `@ai-sdk/openai@^2.0.103` to stay on `@ai-sdk/provider@2.x` (ai@5's peer).

### TODO-020: `helpbase login` — proper device flow (Claude / Supabase CLI pattern)
**Completed:** 2026-04-17 via PR #3 (726a0fd) — `packages/cli/src/lib/auth-device.ts::deviceLogin` implements the full RFC 8628 browser device flow (the same spec `gh auth login`, `claude login`, and `supabase login` use). `packages/cli/src/commands/login.ts` runs it as the default; `--email` is the magic-link fallback for CI/sandboxed envs. State machine covers expiry hints, polling backoff, browser auto-open, and cancel. Better Auth's device plugin powers the server side (`apps/web/app/device/`); the browser-facing `/device` page has its own test coverage via `phase-reducer.test.ts` + `AuthorizeDeviceClient.test.tsx`.
...
## Dropped (rationale retained for future reference)

### TODO-017: `helpbase agent` — prompt-to-PR doc writer
**Dropped:** 2026-04-17 — explicitly blocked on "real user feedback from sync" per its own body. No feedback has arrived because sync itself hasn't been widely used yet. Keeping this as a stale P3 creates false confidence that the idea is captured; dropping it is honest. When sync usage grows and feedback surfaces a real need for prompt-to-PR, re-file with current context.
